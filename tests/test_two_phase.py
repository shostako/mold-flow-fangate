...
def _fan_gate() -> Geometry:
    """Small fan-gate plate standing in for sim's stepped film-gate fixture:
    frame 0.35 / inner 0.50 is the two-band plate, the land band (0.3 → 1.0)
    compresses with it, the fan / well / sprue foot are the fixed block."""
    cfg = FanGatePlateConfig(
        ...
        slug_d_mm=2.0,
        slug_depth_mm=1.0,
        sprue_bottom_d_mm=3.0,
        cell_size_mm=1.0,
        pad_mm=3.0,
    )
    return build_fan_gate_plate_geometry(cfg)
