...
- 井戸 φ23 で `y_gate_end` / `y_edge` / 額縁境界が 1 mm 格子のセル中心にちょうど乗り、
  開区間バンドの空振り（vacuous な `np.all`）と解析体積の半セルずれが発生。境界に触るバンドは閉区間＋
  `.any()` ガードに、厳密な解析体積・表示原点・▽面積の検証は 0.5 mm 格子（全境界がセルエッジ）に変更。
  既にすり抜けていた偽の緑 2 件（`test_without_the_tab` の last_fan/first_plate、傾斜長 0 の last_gate）も同時に修正
